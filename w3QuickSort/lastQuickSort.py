def quicksort_comparison_count_last_pivot(arr):
    # Base case: if the array has one element or is empty, no comparisons needed
    if len(arr) <= 1:
        return 0, arr

    # Before performing Partition, swap the last element (pivot) with the first element
    ...
    left_comparisons, left_sorted = quicksort_comparison_count_last_pivot(low)
    right_comparisons, right_sorted = quicksort_comparison_count_last_pivot(high)
...
total_comparisons_last_pivot, sorted_array_last_pivot = quicksort_comparison_count_last_pivot(input_array)

# Print the result
print("Total number of comparisons using the last element as pivot:", total_comparisons_last_pivot)
